# -*- coding: utf-8 -*-
"""
Script to create a 100% bulletproof main.tex for Overleaf:
1. Native TikZ PRISMA 2020 Flowchart (zero external file dependency for PRISMA).
2. Robust \incluirfigura macro with automatic fallbacks for all image paths.
3. \texorpdfstring for all math in headings (zero hyperref math-shift warnings).
4. \raggedright in minipages (zero underfull \hbox badness).
5. Zero bold in body text.
6. Packages all images + main.tex into scratch, Downloads, Desktop ZIPs.
"""
...
% TikZ para Diagramação Vetorial Nativa
\usepackage{tikz}
...
% Macro para Inclusão Segura de Figuras com Fallback
\newcommand{\incluirfigura}[3][width=0.95\textwidth]{%
  \IfFileExists{figuras/#2}{%
    ...
    \IfFileExists{#2}{%
      ...
      \begin{center}%
        \fbox{\parbox{0.9\textwidth}{\centering\small\textbf{Figura: #2}\\\vspace{0.2cm}\textit{#3}}}%
      \end{center}%
    }%
  }%
}
...
\begin{figure}[htbp]
  \centering
  \incluirfigura[width=0.95\textwidth]{flow3_cho_pipeline.png}{Diagrama Conceitual das Etapas de Formulação Metodológica.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    box/.style={
      ...
      line width=0.8pt, inner sep=5pt, text width=6.3cm, font=\footnotesize, align=left
    },
    exbox/.style={
      rectangle, rounded corners=3pt, draw=red!70!black, fill=red!5, 
      line width=0.8pt, inner sep=5pt, text width=5.0cm, font=\footnotesize, align=left
    },
    phase/.style={
  ...
  \node[phase] (p2) at (-4.2, -3.1) {TRIAGEM};
  \node[phase] (p3) at (-4.2, -6.3) {ELEGIBILIDADE};
  \node[phase] (p4) at (-4.2, -9.5) {INCLUSÃO};
  ...
  \node[exbox, right=0.8cm of id1] (dup) {
...
\subsection{Dedução Formal do Índice de Detectabilidade (\texorpdfstring{$d'$}{d'})}
...
\section{Métricas Espectrais Contínuas no Domínio de Fourier (\texorpdfstring{AAPM TG-233}{AAPM TG-233})}
...
\subsubsection{1. Dedução da Função de Transferência da Tarefa (\texorpdfstring{$TTF(f)$}{TTF(f)})}
...
\subsubsection{2. Dedução do Espectro de Potência do Ruído (\texorpdfstring{$NPS(f)$}{NPS(f)})}
...
print(f"Bulletproof main.tex written. Size: {len(tex_content)} bytes, {len(tex_content.splitlines())} lines")
...
for zip_dest in zip_targets:
    ...
    print(f"Created bulletproof ZIP at: {zip_dest}")
