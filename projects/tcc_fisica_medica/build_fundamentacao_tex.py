...
fundamentacao_tex = r"""\chapter{Fundamentos Teóricos da Qualidade Baseada em Tarefa}
\label{chap:fundamentacao}

\section{Teoria de Detecção de Sinais e Tomada de Decisão Estatística}
\label{sec:sdt}

A Teoria de Detecção de Sinais (SDT), originada na engenharia de telecomunicações e adaptada para a física médica, fornece a estrutura matemática para modelar o processo de diagnóstico médico sob condições de incerteza estocástica\supercite{peterson1954, lusted1968, metz1986, barrett_myers_2004}.
...
A variação do limiar $t_c$ permite construir a curva Característica de Operação do Receptor (curva ROC), cujas ordenadas representam a Fração de Verdadeiros Positivos (TPF ou Sensibilidade) e as abscissas indicam a Fração de Falsos Positivos (FPF ou $1 - \text{Especificidade}$)\supercite{metz1986}:
\begin{equation}
...
Pelo Teorema de Wiener-Khinchin, a Transformada de Fourier bidimensional da função de autocorrelação espacial é exatamente igual à densidade espectral de potência do ruído, denominada Espectro de Potência do Ruído ($NPS(u, v)$)\supercite{barrett_myers_2004, wagner1979}. Essa propriedade permite transpor o cálculo de $d'$ para integrais contínuas no domínio das frequências espaciais.
...
\begin{enumerate}
  ...
  \item Diferenciação numérica radial para obter a Função de Espalhamento de Linha ($\text{LSF}(r) = \frac{d}{dr}\text{ESF}(r)$);
...
O Espectro de Potência do Ruído 2D ($NPS(u, v)$) quantifica como a energia do ruído estocástico se distribui pelas frequências espaciais da imagem\supercite{samei2019}:
\begin{equation}
...
O olho humano não responde de forma uniforme a todas as frequências espaciais. A Função de Sensibilidade ao Contraste do sistema visual atua como um filtro passa-faixa, modelado matematicamente pelo Filtro Ocular $E(f)$\supercite{burgess1994, samei2019}:
\begin{equation}
...
Adicionalmente, a fisiologia da visão introduz um ruído interno neural ($\sigma_{\text{int}}^2$), de modo que a detectabilidade humana real relaciona-se com o modelo computacional por\supercite{burgess1999}:
\begin{equation}
  d'_{\text{humano}} = \frac{d'_{\text{NPWE}}}{\sqrt{1 + \left(\frac{\sigma_{\text{int}}}{\sigma_{\text{ext}}}\right)^2}}
  \label{eq:ruido_interno}
\end{equation}

\subsection{Espectro da Tarefa Diagnóstica e o Critério de Rose}
Para uma lesão circular homogênea de raio $R$ e contraste central $\Delta C$, o espectro da tarefa diagnóstica no domínio de Fourier é descrito por uma função de Bessel ordinária de primeira espécie $J_1$\supercite{burgess1999}:
\begin{equation}
...
\section{A Evolução dos Observadores Lineares}
\label{sec:observadores_lineares}

\subsection{O Observador Ideal Bayesiano e o Limite Superior Termodinâmico}
O Observador Ideal (IO) representa o tomador de decisão estatístico ótimo que utiliza todas as informações físicas do feixe de radiação e do detector para maximizar a área sob a curva ROC ($AUC$), de acordo com o Lema de Neyman-Pearson\supercite{barrett_myers_2004}.
...
\subsection{O Observador NPW e a Dedução Contínua do Modelo NPWE}
Para aproximar os modelos matemáticos das limitações biológicas, desenvolveu-se o modelo sem pré-branqueamento (\emph{Non-Prewhitening} --- NPW), cuja premissa é que o olho humano não inverte matrizes de ruído, atuando como um filtro casado simples $\mathbf{w}_{\text{NPW}} = \mathbf{s}$\supercite{burgess1994}.

A incorporação do filtro ocular $E(f)$ deu origem ao modelo NPWE (\emph{Non-Prewhitening with Eye Filter}), que foi adotado pelo relatório internacional AAPM TG-233 como referência normativa para a avaliação de tomógrafos clínicos\supercite{aapm_tg233_2019}.
...
\subsection{O Desafio dos Fundos Estruturados: Hotelling Observer e Channelized Hotelling Observer}
Quando uma lesão está localizada sobre tecidos anatômicos reais (como o parênquima pulmonar ou o trabeculado ósseo), o fundo introduz uma componente adicional de ruído estrutural. A matriz de covariância total é dada por $\mathbf{K}_{\text{total}} = \mathbf{K}_{\text{ruído}} + \mathbf{K}_{\text{anatômico}}$\supercite{barrett_myers_2004}.
...
Para simular o processamento de imagens realizado pelos neurônios do córtex visual primário (área V1) e contornar o custo computacional da inversão da matriz $\mathbf{K}_{\text{total}}$, Barrett et al.\supercite{barrett1993}, Yao \& Barrett\supercite{yao1992} e Myers \& Barrett\supercite{myers1987} introduziram o Observador de Hotelling Canalizado (\emph{Channelized Hotelling Observer} --- CHO).
...
O fluxo completo de processamento do modelo CHO está ilustrado na \cref{fig:cho_pipeline}.
...
\begin{enumerate}
  ...
  \end{equation}
  \item \textbf{Canais de Gabor:} Funções sinusoidais moduladas por envelope gaussiano com orientação espacial $\theta$ e frequência foveal $f_c$\supercite{abbey2001}.
\end{enumerate}

\subsection{Validação Psicofísica com Leitores Humanos e Metodologia MRMC}
Para certificar a confiabilidade metrológica de um observador computacional, é imperativo comparar seus resultados com o desempenho de médicos radiologistas reais em estudos psicofísicos 2AFC sob análise estatística \emph{Multi-Reader Multi-Case} (MRMC)\supercite{dorfman1992, obuchowski1995, hillis2011, racine2021}.
...
with open(os.path.join(tex_dir, "fundamentacao.tex"), "w", encoding="utf-8") as f:
    f.write(fundamentacao_tex)

print("Fundamentacao written.")
