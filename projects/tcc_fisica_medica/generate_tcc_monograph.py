...
monograph main.tex with zero bold in body text, full analytical discussions with 
clinical problematizations, and 100% compilation safety for Overleaf.
"""
import os
import zipfile
import shutil
...
\begin{itemize}[leftmargin=*,label={}]
  ...
  \item Figura 3.1 -- Diagrama de Fluxo Metodológico da Revisão Sistemática (Protocolo de Busca Booleana e Seleção de Evidências) \dotfill 26
  \item Figura 4.1 -- Modelagem Numérica Sintética da Teoria de Detecção de Sinais (SDT), Curvas ROC Paramétricas e Desempenho Psicofísico 2AFC \dotfill 32
  \item Figura 4.2 -- Modelagem Numérica Sintética das Quatro Funções Espectrais no Domínio de Fourier segundo o Relatório AAPM TG-233 \dotfill 39
  \item Figura 4.3 -- Modelagem Numérica Sintética dos Canais Corticais do Observador CHO e Desempenho em Fundo Anatômico \dotfill 48
  \item Figura 4.4 -- Diagrama do Fluxo de Decisão do Observador de Hotelling Canalizado (CHO) \dotfill 50
  \item Figura 4.5 -- Modelagem Numérica Sintética do Impacto da Não-Linearidade em DLR, Correlação com Radiologistas e Detrending Polinomial 2D \dotfill 55
  \item Figura 4.6 -- Metodologia Experimental com Phantoms Físicos Antropomórficos, Inserção Híbrida de Lesões 3D e Plataforma Psicofísica 2AFC \dotfill 59
  \item Figura 4.7 -- Arquitetura de Redes Neurais para Observadores de Modelo por Aprendizado Profundo (DLMO / Vision Transformers) \dotfill 63
  \item Figura 4.8 -- Modelagem Numérica Sintética da Otimização Multiobjetivo e Fronteira de Pareto Tridimensional $(D, T, -W)$ \dotfill 69
  \item Figura 4.9 -- Estrutura Conceitual do Pipeline Metrológico em Tomografia Computadorizada segundo o Relatório AAPM TG-233 \dotfill 73
\end{itemize}
\clearpage
...
\begin{itemize}[leftmargin=*,label={}]
  ...
  \item Tabela 4.1 -- Resumo Comparativo das Classes de Observadores de Modelo em Física Médica \dotfill 52
  \item Tabela 4.2 -- Síntese das Tecnologias de Detectores em Tomografia Computadorizada (EID vs PCCT) \dotfill 67
\end{itemize}
\clearpage
...
\begin{enumerate}
  ...
  \item Modelagem e Simulação Numérica Sintética em Python: Implementar e documentar exaustivamente as formulações analíticas e os parâmetros matemáticos empregados nas modelagens numéricas computacionais sintéticas desenvolvidas para fins didáticos e demonstrativos de cada curva e gráfico teórico.
\end{enumerate}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{llX}
    ...
    \midrule
    Figura 4.1 & $t_0 \sim \mathcal{N}(\mu_0, \sigma_t^2)$ & $\mu_0 = 0$, $\mu_1 = 2{,}2$, $\sigma_t = 1{,}0$, $t_c = 1{,}3$, \\
    ...
    \addlinespace
    Figura 4.2 & $TTF(f; \Delta C)$ (Richard \& Samei) & Iodo (+300 HU): $f_{50}=0{,}58\text{ mm}^{-1}, \alpha=3{,}2$; \\
    ...
    \addlinespace
    Figura 4.3 & $C_j(f) = e^{-\frac{f^2}{2\sigma_{j1}^2}} - e^{-\frac{f^2}{2\sigma_{j2}^2}}$ & 5 canais D-DOG: $f \in [0{,}10; 0{,}85]\text{ mm}^{-1}$; \\
    ...
    \addlinespace
    Figura 4.5 & $d'(\text{Dose}) \propto \text{Dose}^\beta$ & FBP: $\beta = 0{,}50$; DLR: curva logística não linear; \\
    (DLR e Detrending) & $P_2(x, y) = \sum a_{ij} x^i y^j$ & Detrending: ajuste polinomial 2D via Mínimos Quadrados \\
    \addlinespace
    Figura 4.8 & Variedade 3D $(D, T, -W)$ & Otimização: NSGA-II (100 indivíduos, 200 gerações), \\
...
Este capítulo reúne os principais resultados da revisão sistemática, apresentando as deduções matemáticas rigorosas da SDT e da metrologia no domínio de Fourier, a análise comparativa entre observadores clássicos e profundos, as causas do colapso dos modelos analíticos lineares sob DLR, os fundamentos da PCCT e a sistematização conceitual do fluxo de trabalho do relatório AAPM TG-233. Ao término de cada bloco dedutivo, desenvolve-se uma discussão analítica aprofundada que problematiza as implicações físicas em tarefas clínicas reais e na proteção radiológica dos pacientes.
...
A forma matemática característica do $NPS_{\text{FBP}}(f)$ apresentada no Painel (B) da \cref{fig:spectral_metrics} não é arbitrária; ela decorre diretamente da física do Teorema da Fatia Central (\cref{eq:fourier_slice}) e do filtro de rampa $|f|$ da FBP (\cref{eq:fbp_formula}). 
