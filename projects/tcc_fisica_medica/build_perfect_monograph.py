...
% Formatação dos Títulos de Capítulos e Seções (Objetivos e Claros)
\usepackage{titlesec}
...
A Tomografia Computadorizada (TC) transformou a prática médica ao viabilizar a reconstrução volumétrica do corpo humano com elevada resolução anatômica. Contudo, os avanços recentes introduziram desafios metrológicos de grande relevância: o uso expressivo de radiação ionizante em procedimentos diagnósticos e a substituição das reconstruções clássicas por algoritmos não lineares de inteligência artificial. Este capítulo contextualiza os princípios físicos da formação da imagem tomográfica, expõe detalhadamente as grandezas clássicas e suas premissas, fundamenta a transição para a avaliação baseada em tarefa e define os objetivos desta monografia.

\section{Os dois lados da Tomografia Computadorizada}
\label{sec:paradoxo_tc}

\subsection{Contextualização e Relevância}
Para compreender a necessidade de novos modelos de avaliação da qualidade da imagem, é essencial examinar o princípio físico da atenuação de raios X e o compromisso ético entre benefício diagnóstico e radioproteção.

\subsection{Fundamentos da Formação da Imagem em TC}
A tomografia computadorizada fundamenta-se na transmissão e atenuação de fótons de raios X através da matéria biológica. Considere um feixe monoenergético com intensidade inicial $I_0$ fótons por segundo incidindo sobre um meio material atenuador. De acordo com a Lei de Beer-Lambert-Bouguer, a intensidade transmitida $I$ ao longo de um trajeto retilíneo $L$ é expressa por:
\begin{equation}
...
\begin{itemize}
  ...
  \item $\mu(x, y, z; E)$ representa o coeficiente de atenuação linear do tecido biológico na coordenada espacial $(x, y, z)$ para a energia de fótons $E$, com unidade de $\text{cm}^{-1}$;
...
Para tornar as imagens comparáveis entre diferentes equipamentos e tensões de tubo, os coeficientes lineares reconstruídos $\mu_{\text{tecido}}$ são convertidos para a escala padronizada de Unidades Hounsfield (HU), normalizada em relação à atenuação da água pura nas mesmas condições de irradiação:
\begin{equation}
  \text{Número CT (HU)} = 1000 \times \frac{\mu_{\text{tecido}} - \mu_{\text{água}}}{\mu_{\text{água}}}
  \label{eq:escala_hounsfield}
\end{equation}
Por definição, o ar atmosférico apresenta $-1000\text{ HU}$ ($\mu_{\text{ar}} \approx 0$), a água pura calibrada apresenta exatamente $0\text{ HU}$, o tecido pulmonar arejado varia entre $-800\text{ e }-600\text{ HU}$, o tecido adiposo (gordura) situa-se em torno de $-100\text{ HU}$, o parênquima hepático e muscular varia de $+40\text{ a }+60\text{ HU}$, e o osso cortical denso varia de $+1000\text{ a }+3000\text{ HU}$.

\subsection{O Risco Biológico e a Radioproteção}
...
\begin{enumerate}
  \item \textbf{Efeitos Determinísticos (Reações Teciduais):} Ocorrem apenas após a superação de uma dose limiar (geralmente acima de $0{,}5\text{ a }1{,}0\text{ Gy}$), resultando em morte celular em massa e perda de função tecidual (como eritema cutâneo e catarata);
...
Globalmente, a tomografia computadorizada responde por mais de 60\% de toda a dose de radiação médica administrada à população, embora represente apenas cerca de 10\% a 15\% do volume total de exames radiológicos realizados \cite{mccollough2026, bushberg2020}.

Esse panorama impõe o princípio de radioproteção ALARA (\emph{As Low As Reasonably Achievable} --- Tão Baixo Quanto Razoavelmente Exequível) e atende aos requisitos normativos federais brasileiros (ANVISA RDC 611/2022 e IN 93/2021) \cite{anvisa_rdc611_2022, anvisa_in93_2021}. A missão da física médica reside em reduzir a dose de radiação entregue ao paciente ao nível estritamente necessário para garantir o diagnóstico médico com segurança.
...
\subsection{Contextualização e Relevância}
A metrologia em radiologia dependeu durante décadas de índices escalares simples calculados em objetos homogêneos. Nesta seção, explicam-se rigorosamente essas grandezas físicas clássicas, suas premissas teóricas subjacentes e as razões físicas de seu colapso frente aos novos algoritmos de inteligência artificial.

\subsection{Definição Detalhada das Grandezas Físicas Clássicas}
Historicamente, o controle de qualidade em TC fundamentou-se em quatro parâmetros principais:
\begin{enumerate}
...
\subsection{Premissas Teóricas Subjacentes: Linearidade, Isoplanatismo e WSS}
A validade matemática das métricas clássicas depende de três premissas estritas:
\begin{enumerate}
...
\subsection{Conceitos Prévios Essenciais: Simuladores, Filtros e Paradigmas}
...
\subsection{A Ruptura Metrológica das Reconstruções Modernas}
...
\subsection{Contextualização e Relevância}
Esta seção apresenta a estrutura formal do paradigma TBIQ, articulando como a física dos detectores, o processamento espectral e a percepção visual integram-se no Índice de Detectabilidade ($d'$).

\subsection{Conceito e Pilares do TBIQ}
...
\subsection{Transição}
Definido o panorama físico e conceitual, apresentam-se a seguir os objetivos que norteiam o desenvolvimento deste trabalho.

\section{Objetivos}
\label{sec:objetivos}

\subsection{Objetivo Geral}
Desenvolver uma formulação teórica, biofísica e computacional unificada dos modelos perceptivos de qualidade de imagem baseada em tarefa em tomografia computadorizada, estabelecendo as bases metrológicas para a avaliação de tecnologias avançadas e otimização de protocolos clínicos.
...
\subsection{Contextualização e Relevância}
A detecção de uma patologia em um exame médico com ruído é formalmente descrita como um problema estocástico de detecção de sinal sob ruído. A SDT estabelece a base matemática para analisar decisões diagnósticas objetivamente.

\subsection{Formalismo do Teste Binário de Hipóteses}
A Teoria de Detecção de Sinais (SDT), introduzida na física matemática por Peterson, Birdsall e Fox (1954) e aplicada à medicina por Lusted (1968) e Metz (1986), analisa o processo de decisão diagnóstica sob flutuações estocásticas \cite{peterson1954, lusted1968, metz1986}.
...
\subsection{Análise Detalhada dos Gráficos da Figura 2.1}
...
\subsection{Contextualização e Relevância}
O índice $d'$ é a grandeza escalar unificadora do paradigma TBIQ. Compreender sua derivação permite comparar equipamentos, protocolos de dose e algoritmos computacionais sob a mesma escala física.
...
\subsection{Contextualização e Relevância}
A avaliação de radiologistas exige métodos que isolem a acurácia diagnóstica de vieses comportamentais. Esta seção formaliza a equivalência entre a Área sob a Curva ROC e a taxa de acerto no teste 2AFC.

\subsection{Equivalência Matemática entre ROC e 2AFC}
...
\subsection{Análise Detalhada dos Gráficos da Figura 2.2}
...
\subsection{Formulações Matemáticas das Quatro Funções Espectrais}
\begin{enumerate}
...
\subsection{Contextualização e Relevância}
Para quantificar o desempenho de um sistema tomográfico, é fundamental estabelecer o teto teórico absoluto de informação diagnóstica permitido pelas leis da física. O Observador Ideal Bayesiano define esse limite superior fundamental.

\subsection{Dedução Matemática do Observador de Hotelling}
O Observador Ideal (IO) fundamenta-se na razão de verossimilhança de Bayes $\Lambda(\mathbf{g}) = \frac{p(\mathbf{g}|H_1)}{p(\mathbf{g}|H_0)}$. Quando o ruído de fundo segue uma distribuição normal multivariada com matriz de autocovariância $\mathbf{K}$, a razão de verossimilhança logarítmica reduz-se ao Observador de Hotelling (HO) \cite{barrett_myers_2004, wagner1979}:
\begin{equation}
...
\subsection{Contextualização e Relevância}
O modelo NPWE é a formulação analítica mais utilizada em controle de qualidade de rotina em simuladores homogêneos de TC. Esta seção deduz sua integral contínua em frequência.

\subsection{Dedução da Integral Contínua do NPWE}
...
\subsection{Contextualização e Relevância}
Na rotina clínica, as lesões patológicas estão imersas em complexas anatomias estruturadas. O modelo CHO emula o processamento da área V1 do córtex visual humano para permitir a inversão estável da covariância anatômica.
...
Em imagens clínicas com matriz $N = 128 \times 128 = 16.384$ pixels, a matriz de covariância anatômica $\mathbf{K}_{\mathbf{b}} \in \mathbb{R}^{N \times N}$ possui mais de 268 milhões de elementos, inviabilizando sua inversão numérica direta \cite{myers1987, yao1992}.
...
\subsection{Análise Detalhada dos Gráficos da Figura 3.2}
...
\subsection{Contextualização e Relevância}
Compreender as diferenças de arquitetura e treinamento das redes neurais de cada fabricante é essencial para construir uma metodologia de avaliação de qualidade neutra em relação a fornecedores (*vendor-neutral*).

\subsection{Gerações de Reconstrução Tomográfica}
A evolução dos algoritmos tomográficos compreende quatro etapas:
\begin{enumerate}
...
\subsection{Contextualização e Relevância}
Nesta seção, analisa-se como as ativações não lineares de redes profundas provocam a quebra da linearidade, a perda de estacionariedade do ruído e o efeito ceroso prejudicial à detecção de pequenas patologias.

\subsection{Manifestações Físicas da Não-Linearidade em DLR}
...
\subsection{Análise Detalhada dos Gráficos da Figura 4.1}
...
\subsection{Etapas do Pipeline com Phantoms Híbridos}
...
\subsection{Contextualização e Relevância}
Variações anatômicas naturais (como transições pleuropulmonares) contaminam o espectro de ruído com frequências ultrabaixas. O detrending polinomial remove essa contaminação com rigor matemático.

\subsection{Formulação Matemática do Detrending e Bootstrap}
...
\subsection{Arquitetura Neural Vision Transformer e Auto-Atenção}
...
\subsection{Contextualização e Relevância}
Um modelo de inteligência artificial deve generalizar robustamente entre diferentes fornecedores tomográficos e manter fidelidade com o julgamento médico humano.

\subsection{Perda Perceptual Multitarefa e Validação LOSO}
...
\subsection{Detectores de Conversão Direta e Imagens Monoenergéticas (VMI)}
...
\subsection{Contextualização e Relevância}
Na rotina clínica de emergências e centros de trauma, a otimização não pode considerar apenas a dose e a detectabilidade: o tempo operacional total ($T$) é uma variável crítica para a sobrevida do paciente. Esta seção formaliza a otimização simultânea dessas três grandezas.

\subsection{Formulação da Programação Não Linear Multiobjetivo}
A otimização de protocolos tomográficos é formulada como um problema de minimização vetorial multiobjetivo \cite{oostveen2021}:
\begin{equation}
...
\subsection{Análise Detalhada dos Gráficos da Figura 5.2}
...
\subsection{Contextualização e Relevância}
A padronização metrológica rigorosa assegura que as medições de detectabilidade sejam diretamente reprodutíveis em qualquer centro de pesquisa internacional.

\subsection{Parâmetros de Aquisição e Modelagem de Lesões}
As aquisições tomográficas seguem as diretrizes da AAPM \cite{aapm_tg233_2019}:
\begin{itemize}
...
\subsection{Contextualização e Relevância}
A condução de experimentos psicofísicos com humanos requer conformidade ética estrita para assegurar o consentimento livre e a proteção dos dados dos participantes.

\subsection{Diretrizes do Sistema CEP/CONEP}
Os testes psicofísicos 2AFC com médicos radiologistas cumprem as diretrizes éticas:
\begin{itemize}
...
\subsection{Transição}
Com a arquitetura computacional e as salvaguardas éticas estabelecidas, o capítulo seguinte sintetiza as conclusões desta monografia.
...
Este capítulo final sintetiza as contribuições científicas desenvolvidas nesta monografia, analisa o impacto clínico e normativo para os serviços de radiologia e apresenta as perspectivas futuras da física médica na garantia da qualidade tomográfica.

\section{Síntese da Trajetória Biofísica e Metrológica}
\label{sec:sintese_biofisica}

\subsection{Contextualização e Relevância}
A evolução da garantia da qualidade em tomografia computadorizada reflete a transição contínua de parâmetros escalares simplificados para a modelagem rigorosa da física quântica e da neurobiologia da percepção visual.

\subsection{As Quatro Fases da Avaliação de Qualidade}
A trajetória percorrida nesta monografia consolida quatro marcos científicos:
\begin{enumerate}
  \item \textbf{Fase Linear Analítica (1950--1990):} Fundamentada na Teoria de Detecção de Sinais e no Observador Ideal Bayesiano, introduzindo filtros de sensibilidade ao contraste (NPWE) para imagens homogêneas;
  \item \textbf{Modelagem Cortical (1990--2015):} Desenvolvimento do Observador CHO com canais corticais de frequência para tratar o ruído anatômico estruturado;
  \item \textbf{Ruptura da Linearidade (2015--2026):} A introdução de redes neurais DLR quebrou as premissas de linearidade e estacionariedade (WSS), demonstrando a obsolescência das grandezas clássicas ($SNR$, $CNR$, $\sigma_{\text{HU}}$);
  \item \textbf{Paradigma da IA Perceptual e Otimização Multiobjetivo (2026+):} Consolidação dos observadores DLMO baseados em \emph{Vision Transformers} com auto-atenção multi-cabeça, calibrados diretamente contra radiologistas em testes 2AFC/MRMC e integrados à Fronteira de Pareto Tridimensional $(D, T, -W)$.
\end{enumerate}

\subsection{Transição}
A consolidação dessa metodologia produz impactos diretos na segurança dos pacientes e na rotina hospitalar.

\section{Impacto Clínico, Operacional e Normativo}
\label{sec:impacto_clinico}

\subsection{Benefícios para a Física Médica e a Saúde Pública}
A implementação do pipeline metrológico acarreta benefícios práticos fundamentais:
\begin{itemize}
  \item \textbf{Segurança Radiológica Personalizada:} Comprovação matemática de que reduções expressivas de dose em protocolos pediátricos e de rastreio oncológico preservam integralmente a detectabilidade diagnóstica;
  \item \textbf{Auditoria Hospitalar e Conformidade Normativa:} Automação dos testes de aceitação e controle de qualidade segundo o relatório AAPM TG-233, ANVISA RDC 611/2022 e auditoria internacional IAEA 5-Star \cite{iaea_5star_2026};
  \item \textbf{Harmonização de Parques Tecnológicos Multimarca:} Equalização do desempenho diagnóstico entre tomógrafos de diferentes fornecedores (GE, Siemens, Canon e Philips) em grandes complexos hospitalares.
\end{itemize}

\subsection{Considerações Finais do Trabalho}
Esta monografia de conclusão de curso consolida as bases teóricas, biofísicas e computacionais necessárias para a transição definitiva da metrologia tomográfica para a era da inteligência artificial e da contagem de fótons. Conclui-se que a união entre a física de radiações, o processamento estocástico de sinais e os modelos perceptivos constitui o caminho definitivo para assegurar o uso seguro e eficiente da radiação ionizante em benefício da saúde humana.
"""
...
print("Monografia enriquecida com explicações profundas de grandezas e análise detalhada por painel gerada com sucesso!")
