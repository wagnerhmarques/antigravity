...
<center>
<h1><b>WAGNER H. M.</b></h1>
<br><br><br><br>
<h2><b>MODELOS PERCEPTIVOS NA AVALIAÇÃO DA QUALIDADE DE IMAGEM EM TOMOGRAFIA COMPUTADORIZADA: DA TEORIA CLÁSSICA DE DETECÇÃO DE SINAIS AOS MODELOS DE APRENDIZADO PROFUNDO E OTIMIZAÇÃO MULTIOBJETIVO</b></h2>
</center>

<br><br><br><br><br><br>

<center>
<h3><b>SÃO PAULO</b><br><b>2026</b></h3>
</center>
...
<div style="margin-left: 45%; text-align: justify; font-size: 0.9em;">
...
<center>
<h2><b>FOLHA DE APROVAÇÃO</b></h2>
<br>
<h3><b>WAGNER H. M.</b></h3>
<br>
...
A Tomografia Computadorizada (TC) ocupa posição central no diagnóstico médico moderno, operando sob o permanente compromisso físico entre a minimização da dose de radiação ionizante e a preservação do desempenho diagnóstico. Historicamente, a metrologia e a garantia de qualidade em TC fundamentaram-se em grandezas físicas escalares lineares, como a Relação Sinal-Ruído (*Signal-to-Noise Ratio* - SNR), a Relação Contraste-Ruído (*Contrast-to-Noise Ratio* - CNR) e a Função de Transferência de Modulação (*Modulation Transfer Function* - MTF), quantificadas em simuladores geométricos homogêneos. No entanto, a incorporação clínica de algoritmos avançados de reconstrução não lineares — destacando-se as reconstruções iterativas e, fundamentalmente, as reconstruções baseadas em aprendizado profundo (*Deep Learning Image Reconstruction* - DLR) — quebrou as premissas de linearidade e invariância translacional do sistema. Sob regimes não lineares, o ruído da imagem tornou-se espacialmente heterogêneo, dependente da dose, do contraste e da geometria local da cena, induzindo alterações texturais (como a aparência excessivamente suavizada ou *plastic/waxy look*) que não são capturadas pelas métricas tradicionais. Para superar essa limitação, a física médica consolidou o paradigma da Qualidade de Imagem Baseada em Tarefa (*Task-Based Image Quality* - TBIQ), ancorado na Teoria de Detecção de Sinais (*Signal Detection Theory* - SDT), no qual a qualidade da imagem é formalmente definida pelo desempenho de um observador (médico ou modelo matemático) na execução de uma tarefa diagnóstica clínica específica, expressa pelo Índice de Detectabilidade ($d'$). Este trabalho apresenta uma investigação aprofundada e estruturada da evolução dos observadores de modelo (*model observers*). Analisa-se a transição do Observador Ideal Bayesiano para os modelos lineares antropomórficos com filtro ocular (NPWE) e canais corticais de frequência (CHO), demonstrando suas deduções matemáticas contínuas no domínio de Fourier e evidenciando os limites biofísicos que causam seu colapso sob reconstruções DLR e fundos anatômicos complexos. Em resposta, discute-se a fronteira científica representada pelos Observadores Baseados em Aprendizado Profundo (*Deep Learning Model Observers* - DLMO), com arquiteturas neurais de auto-atenção (*Vision Transformers*) calibradas diretamente contra leituras psicofísicas de radiologistas em experimentos de Escolha Forçada entre Duas Alternativas (2AFC) sob modelagem *Multi-Reader Multi-Case* (MRMC). Por fim, formaliza-se a integração desses modelos à Otimização Multiobjetivo, expandindo o compromisso bidimensional tradicional para uma Fronteira de Pareto Tridimensional $(D, T, -W)$, que incorpora o tempo operacional ($T$) de aquisição e reconstrução junto à dose ($D$) e à detectabilidade ($W$). Esta monografia consolida as bases teóricas, computacionais e metrológicas que apoiam o desenvolvimento de protocolos tomográficos otimizados e orientam a pesquisa de Doutorado Direto do autor no IFUSP.

**Palavras-chave:** Tomografia Computadorizada. Qualidade de Imagem Baseada em Tarefa. Observadores de Modelo. Índice de Detectabilidade. Reconstrução por Aprendizado Profundo. Simuladores Antropomórficos. Otimização Multiobjetivo. Fronteira de Pareto.
...
| $R$ | Raio físico da lesão simulada ($\text{mm}$) |
...
3. [3 A EVOLUÇÃO DOS OBSERVADORES DE MODELO LINEARES](#3-a-evolução-dos-observadores-de-modelo-lineares)
   ...
   - 3.4 [Validação Psicofísica com Leitores Humanos e Metodologia MRMC](#34-validação-psicofísica-com-leitores-humanos-e-metodologia-mrmc)
4. [4 O COLAPSO DA LINEARIDADE E OS SIMULADORES ANTROPOMÓRFICOS](#4-o-colapso-da-linearidade-e-os-simuladores-antropomórficos)
   ...
   - 4.2 [A Quebra da Linearidade, Não-Estacionariedade e a Textura Cerosa](#42-a-quebra-da-linearidade-não-estacionariedade-e-a-textura-cerosa)
   ...
   - 4.4 [Tratamento de Ruído em Anatomias Complexas: Detrending Polinomial e Incerteza Bootstrap](#44-tratamento-de-ruído-em-anatomias-complexas-detrending-polinomial-e-incerteza-bootstrap)
5. [5 O ESTADO DA ARTE: OBSERVADORES POR APRENDIZADO PROFUNDO E OTIMIZAÇÃO MULTIOBJETIVO](#5-o-estado-da-arte-observadores-por-aprendizado-profundo-e-otimização-multiobjetivo)
   - 5.1 [Observadores Baseados em Aprendizado Profundo](#51-observadores-baseados-em-aprendizado-profundo)
   ...
   - 5.3 [Otimização Multiobjetivo em TC: A Fronteira de Pareto Tridimensional](#53-otimização-multiobjetivo-em-tc-a-fronteira-de-pareto-tridimensional)
   - 5.4 [Novas Tecnologias: Tomografia por Contagem de Fótons e Imagens Monoenergéticas](#54-novas-tecnologias-tomografia-por-contagem-de-fótons-e-imagens-monoenergéticas)
...
7. [7 CONSIDERAÇÕES FINAIS E PERSPECTIVAS](#7-considerações-finais-e-perspectivas)
   ...
   - 7.3 [Articulação com a Pesquisa de Doutorado Direto](#73-articulação-com-a-pesquisa-de-doutorado-direto)
...
A Tomografia Computadorizada revolucionou a medicina diagnóstica desde a sua introdução clínica na década de 1970 por Godfrey Hounsfield. Ao permitir a reconstrução tridimensional de secções transversais do corpo humano com elevada diferenciação de densidades de tecidos moles e resolução espacial submilimétrica, a TC consolidou-se como a modalidade de escolha para o estadiamento oncológico, o planejamento cirúrgico e radioterápico, a avaliação de traumas agudos e o rastreamento precoce de doenças pulmonares e vasculares (MCCOLLOUGH et al., 2026).

Contudo, o princípio físico basilar da formação da imagem tomográfica reside na atenuação exponencial de feixes de raios X transmitidos através do paciente. A absorção e o espalhamento dessa radiação ionizante no tecido biológico provocam ionizações atômicas e quebras de ligações moleculares em estruturas de DNA celular, associando-se a riscos estocásticos de carcinogênese a longo prazo. Consequentemente, embora represente apenas de 10% a 15% do total de procedimentos radiológicos executados mundialmente, a TC responde por aproximadamente 65% a 70% de toda a dose coletiva de radiação ionizante de origem médica absorvida pela população humana (IAEA, 2026; MCCOLLOUGH et al., 2026).

Essa assimetria impõe a aplicação rigorosa do princípio fundamental da radioproteção: o princípio ALARA (*As Low As Reasonably Achievable*). De acordo com essa diretriz, os protocolos tomográficos devem ser continuamente otimizados para operar na menor dose de radiação compatível com o objetivo diagnóstico desejado.

Para compreender a dificuldade física dessa otimização, é necessário analisar como os fótons de raios X se comportam estatisticamente. A detecção de fótons é um processo estocástico regido pela distribuição de Poisson. Em termos intuitivos, quanto menor o número de fótons emitidos pelo tubo de raios X, maiores são as flutuações estatísticas percentuais registradas pelos detectores. No domínio da imagem reconstruída, essa incerteza manifesta-se visualmente sob a forma de ruído quântico.
...
Durante quatro décadas, o controle de qualidade e a avaliação de desempenho de tomógrafos hospitalares apoiaram-se em grandezas físicas escalares derivadas da teoria de sistemas lineares e invariantes no espaço:
...
b) Analisar a trajetória dos observadores lineares (IO, NPW, NPWE, HO e CHO), explicitando as aproximações do córtex visual e os métodos psicofísicos de validação humana (2AFC e MRMC);
c) Investigar o colapso da linearidade em sistemas DLR, demonstrando o surgimento da não-estacionariedade e justificando a necessidade de simuladores antropomórficos híbridos e técnicas de *detrending*;
d) Examinar a fronteira científica dos Observadores Baseados em Aprendizado Profundo (DLMO), sua calibração com radiologistas e validação de transferibilidade inter-scanners;
e) Formular o problema de Otimização Multiobjetivo em TC através da Fronteira de Pareto Tridimensional $(D, T, -W)$, integrando dose ($D$), tempo operacional ($T$) e detectabilidade ($W$).

## 1.5 Estrutura do Trabalho

Esta monografia está organizada em sete capítulos principais: o Capítulo 2 estabelece os fundamentos teóricos da SDT e a dedução das métricas de Fourier; o Capítulo 3 aborda a era dos observadores lineares; o Capítulo 4 analisa a quebra da linearidade pelas reconstruções modernas e o uso de simuladores antropomórficos; o Capítulo 5 discute os observadores de aprendizado profundo e a otimização multiobjetivo; o Capítulo 6 detalha a arquitetura computacional, a metrologia experimental e os aspectos bioéticos; e o Capítulo 7 sintetiza as conclusões e os direcionamentos futuros da pesquisa.
...
A Teoria de Detecção de Sinais (SDT), originada na engenharia de telecomunicações e adaptada para a física médica, fornece a estrutura matemática para modelar o processo de diagnóstico médico sob condições de incerteza estocástica (METZ, 1986).
...
A manipulação da matriz de covariância $\mathbf{K}$ no domínio espacial torna-se computacionalmente custosa para imagens de alta resolução (onde $N \approx 262.144$ pixels).
...
Pelo Teorema de Wiener-Khinchin, a Transformada de Fourier bidimensional da função de autocorrelação espacial é exatamente igual à densidade espectral de potência do ruído, denominada Espectro de Potência do Ruído ($NPS(u, v)$). Essa propriedade permite transpor o cálculo de $d'$ para integrais contínuas no domínio das frequências espaciais.
...
A Função de Transferência da Tarefa ($TTF(f)$) quantifica a resolução espacial do tomógrafo para um contraste radiológico e nível de dose específicos. Ela é obtida experimentalmente através da técnica da borda circular em insertos cilíndricos de calibração (Painel A da Figura 2).
...
O padrão-ouro experimental para medir a percepção humana em física médica é o experimento de Escolha Forçada entre Duas Alternativas (2AFC).
...
# 3 A EVOLUÇÃO DOS OBSERVADORES DE MODELO LINEARES

## 3.1 O Observador Ideal Bayesiano e o Limite Superior Termodinâmico

O Observador Ideal (IO) representa o tomador de decisão estatístico ótimo que utiliza todas as informações físicas do feixe de radiação e do detector para maximizar a área sob a curva ROC ($AUC$), de acordo com o Lema de Neyman-Pearson.
...
Para aproximar os modelos matemáticos das limitações biológicas, desenvolveu-se o modelo sem pré-branqueamento (*Non-Prewhitening* - NPW), cuja premissa é que o olho humano não inverte matrizes de ruído, atuando como um filtro casado simples $\mathbf{w}_{\text{NPW}} = \mathbf{s}$.
...
Quando uma lesão está localizada sobre tecidos anatômicos reais (como o parênquima pulmonar ou o trabeculado ósseo), o fundo introduz uma componente adicional de ruído estrutural. A matriz de covariância total é dada por $\mathbf{K}_{\text{total}} = \mathbf{K}_{\text{ruído}} + \mathbf{K}_{\text{anatômico}}$.
...
Para simular o processamento de imagens realizado pelos neurônios do córtex visual primário (área V1) e contornar o custo computacional da inversão da matriz $\mathbf{K}_{\text{total}}$, Barrett et al. (1993) e Yao & Barrett (1992) introduziram o Observador de Hotelling Canalizado (*Channelized Hotelling Observer* - CHO).
...
3. **Canais de Gabor:** Funções sinusoidais moduladas por envelope gaussiano com orientação espacial $\theta$ e frequência foveal $f_c$.

## 3.4 Validação Psicofísica com Leitores Humanos e Metodologia MRMC

Para certificar a confiabilidade metrológica de um observador computacional, é imperativo comparar seus resultados com o desempenho de médicos radiologistas reais em estudos psicofísicos 2AFC sob análise estatística *Multi-Reader Multi-Case* (MRMC) (HILLIS et al., 2011).

A modelagem MRMC isola a variabilidade aleatória introduzida pela amostragem de pacientes da variabilidade inerente à interpretação de diferentes médicos, exigindo que o coeficiente de correlação intraclasse ($ICC$) atinja $ICC \ge 0{,}90$ para que o modelo computacional seja aceito como substituto confiável da leitura humana.

<div style="page-break-after: always;"></div>

---

# 4 O COLAPSO DA LINEARIDADE E OS SIMULADORES ANTROPOMÓRFICOS
...
| **Philips Healthcare** | iDose4 | IMR (*Iterative Model Reconstruction*) | **Precise Image** (baseado em redes convolucionais) |
...
## 4.2 A Quebra da Linearidade, Não-Estacionariedade e a Textura Cerosa

Em sistemas tomográficos que utilizam reconstruções por aprendizado profundo (DLR) e MBIR, a relação matemática entre as projeções brutas $\mathbf{y}$ e a imagem final $\mathbf{x}$ é estritamente não linear:
...
## 4.4 Tratamento de Ruído em Anatomias Complexas: Detrending Polinomial e Incerteza Bootstrap
...
# 5 O ESTADO DA ARTE: OBSERVADORES POR APRENDIZADO PROFUNDO E OTIMIZAÇÃO MULTIOBJETIVO

## 5.1 Observadores Baseados em Aprendizado Profundo

Para superar a quebra de linearidade dos algoritmos DLR, a física médica desenvolveu os Observadores de Modelo Baseados em Aprendizado Profundo (*Deep Learning Model Observers* - DLMO).
...
A auto-atenção permite à rede computacional aprender correlações espaciais globais e locais simultaneamente, emulando os mecanismos fisiológicos de atenção foveal e periférica observados em radiologistas especialistas.
...
## 5.3 Otimização Multiobjetivo em TC: A Fronteira de Pareto Tridimensional

A otimização tradicional de protocolos limitava-se a buscar o equilíbrio entre dose e qualidade de imagem. Contudo, em ambientes hospitalares de alta rotatividade (prontos-socorros e centros de trauma), o tempo operacional total ($T = T_{\text{aq}} + T_{\text{rec}}$) é uma variável determinante.
...
## 5.4 Novas Tecnologias: Tomografia por Contagem de Fótons e Imagens Monoenergéticas

A Tomografia Computadorizada por Contagem de Fótons (*Photon-Counting CT* - PCCT) substitui os detectores cintiladores integradores por semicondutores de conversão direta (CdTe ou Silício), eliminando o ruído eletrônico em baixas doses e fornecendo resolução espacial ultra-alta.

Além disso, a PCCT permite a síntese de Imagens Monoenergéticas Virtuais ($VMI$) em energias selecionáveis de 40 a 140 keV. Em baixas energias (40 a 50 keV), maximiza-se o contraste fotoelétrico do iodo vascular ($TTF$ elevada), enquanto em energias intermediárias (65 a 75 keV) minimiza-se o ruído relativo ($NPS$), permitindo otimizar a detectabilidade $d'$ para cada tipo de lesão clínica (PIMENTA; COSTA, 2025; PIMENTA, 2026).
...
As aquisições tomográficas para calibração seguem as diretrizes internacionais da AAPM:
...
## 7.3 Articulação com a Pesquisa de Doutorado Direto

Esta monografia de conclusão de curso cumpre o papel fundamental de consolidar o embasamento teórico, matemático e computacional que sustenta o projeto de pesquisa de Doutorado Direto do autor (FAPESP 2026–2030) no Grupo de Dosimetria e Radioproteção em Física Médica do IFUSP:
...
b) Execução do estudo psicofísico nacional 2AFC com mais de 60 radiologistas especialistas;
...
d) Mapeamento experimental completo da Fronteira de Pareto Tridimensional $(D, T, -W)$ para os principais protocolos tomográficos de crânio, tórax e abdome.

Conclui-se, assim, este trabalho acadêmico com a certeza de que a física médica brasileira continua contribuindo para a vanguarda científica internacional, unindo o rigor analítico da física à missão de preservar vidas humanas.
...
print(f"ABNT TCC document written successfully to {target_path}")
